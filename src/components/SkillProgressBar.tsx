...
import { Zap, Award, Sparkles, Flame, Shield } from 'lucide-react';
...
export const SkillProgressBar: React.FC<Props> = ({
  ...
  const getBarColor = () => {
    if (progress.isMaxLevel) {
      return 'bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 shadow-[0_0_8px_rgba(251,191,36,0.6)]';
    }
    if (skill.category === 'Manas') {
      ...
      <div className="w-full space-y-1 font-mono">
        <div className="flex items-center justify-between text-[9px]">
          <span className="text-slate-400 font-bold flex items-center gap-1">
            <span className={`px-1 py-0.2 rounded-xs font-black ${
              progress.isMaxLevel
            ...
            <span className="text-slate-500">{progress.stageTitle}</span>
          </span>
          <span className="text-slate-300 font-bold">
            {progress.isMaxLevel ? 'MAX' : `${progress.exp}/${progress.maxExp} XP`} ({progress.percent}%)
          </span>
        </div>
      ...
      {showLabels && (
        <div className="flex items-center justify-between text-[10px]">
          <div className="flex items-center gap-1.5">
            <span className={`px-1.5 py-0.5 rounded-xs font-black uppercase tracking-wider text-[10px] border flex items-center gap-1 ${
              progress.isMaxLevel
