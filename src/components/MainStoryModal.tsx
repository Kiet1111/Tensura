...
export const MainStoryModal: React.FC<Props> = ({ storyState, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/90 backdrop-blur-md animate-fade-in font-sans">
      <motion.div
      ...
      >
        {/* Top Glowing Border */}
        <div className="h-1 bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-400" />
        ...
        <div className="p-3.5 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between gap-2 font-mono text-xs shrink-0">
          <div className="flex items-center gap-2">
            ...
            <strong className="text-white text-sm">{storyState.currentArc}</strong>
          </div>
        ...
        <div className="flex border-b border-slate-800 bg-slate-950 font-mono text-xs shrink-0">
          ...
          >
            ...
            <span>Mốc Cốt Truyện Tensura</span>
          </button>
          ...
          >
            ...
            <span>Quan Hệ Nhân Vật ({storyState.relations.length})</span>
          </button>
          ...
          >
            ...
            <span>Nhật Ký Hành Trình ({storyState.recentCanonChanges.length})</span>
          </button>
        </div>
        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex-1 custom-scrollbar space-y-4">
          {activeTab === 'timeline' && (
            <div className="space-y-3">
              ...
              <div className="space-y-3">
                {storyState.milestones.map((m, idx) => (
                  <div
                    key={m.id}
          ...
          {activeTab === 'relations' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {storyState.relations.map((rel, idx) => (
                <div
                  key={`${rel.name}_${idx}`}
          ...
          {activeTab === 'impact_log' && (
            <div className="space-y-2.5">
              ...
              {storyState.recentCanonChanges.map((change, idx) => (
                <div
                  key={idx}
